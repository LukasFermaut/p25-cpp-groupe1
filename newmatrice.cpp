...
public:
    ...
    Matrix& operator=(const Matrix &m){ //fonction qui permet de mettre m6=m5
        if (this!= &m){
            decr_buffer() //on enleve un du compteur de buffer de m6
            rows=m.rows; //on egalise
            columns=m.columns;
            buffer_ptr=m.buffer_ptr;
            buffer_ptr->incr_counter(); //on ajoute un au compteur de m7
        }
        return *this
    }
    void print()
...
// g++ -g new....
//valgrind ... et on peut suivre les étpaes